  ...
  return (
    <main className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
      <section className="rounded-[28px] border border-border bg-surface px-6 py-6 backdrop-blur">
        <p className="text-sm font-medium uppercase tracking-[0.22em] text-accent">Scan Receive</p>
        <h1 className="mt-2 text-3xl font-semibold tracking-tight">Receive session preview</h1>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-muted">
          Use active session QR tokens to open the public mobile receive form. The next UI step is
          turning this page into a proper session creator with QR rendering.
        </p>
      ...
      <section className="rounded-[28px] border border-border bg-surface px-6 py-6 backdrop-blur">
        ...
        <div className="mt-5 space-y-3 text-sm text-muted">
          <div className="rounded-3xl border border-border bg-surface-strong px-5 py-4">
            ...
            <p className="mt-2 font-mono text-xs">POST /api/workflows/receive-sessions</p>
          </div>
          <div className="rounded-3xl border border-border bg-surface-strong px-5 py-4">
            <p className="font-semibold text-foreground">Submit receive request</p>
            <p className="mt-2 font-mono text-xs">POST /api/workflows/receive-requests</p>
          </div>
        </div>
      </section>
    </main>
